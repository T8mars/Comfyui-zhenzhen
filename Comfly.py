    ...
    def process(self, prompt, model, apikey="", seconds="15", size="1280x720", image=None, seed=0, private=True, skip_error=False):
        ...
        try:
            ...
            while attempts < max_attempts:
                ...
                try:
                    status_response = requests.get(
                        f"https://ai.t8star.org/v1/videos/{task_id}",
                        headers=self.get_headers(),
                        timeout=self.timeout
                    )
...
class Comfly_sora2_new:
    ...
    def __init__(self):
        self.api_key = ""
        self.base_url = "https://t8.ainewsto.com"
        self.timeout = 900
        self.session = requests.Session()
        self.model_mapping = {
            "sora-2": "sy_8",
    ...
    def process(self, prompt, model, num_images=1, seed=0, image_way="image",
        ...
        try:
            ...
            if "queue.fal.run" in response_url:
                response_url = response_url.replace("https://queue.fal.run", "https://ai.t8star.org")
...
class OpenAISoraAPIPlus:
    ...
    - 超时时间：600 秒（10 分钟）
    输入参数：
      - base_url: 默认 https://ai.t8star.org/v1
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "base_url": ("STRING", {"default": "https://ai.t8star.org/v1", "multiline": False}),
...
class Comfly_LLm_API:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "api_baseurl": ("STRING", {"multiline": True, "default": "https://ai.t8star.org/v1"}),
                "api_key": ("STRING", {"default": ""}),
...
class Comfly_seedance2_fal:
    """Seedance 2.0 Reference-to-Video via fal.ai Queue API.
    Uses https://ai.t8star.org/fal as proxy for https://queue.fal.run.
    Generate video from up to 9 images, 3 videos, and 3 audio clips.
    ...
    def upload_image_to_get_url(self, image_tensor):
        ...
        try:
            ...
            if 'url' in result:
                return result['url']
            print(f"[seedance2_fal] Unexpected file upload response: {result}")
            return None
        except Exception as e:
            print(f"[seedance2_fal] Error uploading image: {str(e)}")
            return None
    ...
    def process(self, prompt, image1=None, image2=None, image3=None,
        ...
        try:
            ...
            # Process input images (up to 9)
            all_images = [image1, image2, image3, image4, image5, image6, image7, image8, image9]
            image_urls = []
            input_images = [img for img in all_images if img is not None]
            ...
            if input_images:
                ...
                for idx, img in enumerate(input_images):
                    ...
                    if img_data:
                        image_urls.append(img_data)
                        print(f"[seedance2_fal] Image {idx+1}/{len(input_images)} prepared")
                    pbar.update_absolute(10 + int((idx + 1) / len(input_images) * 10))
            ...
            print(f"[seedance2_fal] Queued, request_id={request_id}, polling (timeout={poll_interval*max_poll_attempts}s)...")
            ...
            for attempt in range(max_poll_attempts):
                ...
                try:
                    ...
                    if attempt % 10 == 0:
                        print(f"[seedance2_fal] Polling... attempt {attempt+1}/{max_poll_attempts}")

                except requests.exceptions.RequestException as e:
                    print(f"[seedance2_fal] Poll error: {e}")
                    continue
            ...
            error_message = f"Error: {str(e)}"
            print(f"[seedance2_fal] {error_message}")
            import traceback
            traceback.print_exc()
            if not skip_error:
                raise
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "Comfly_gpt_image_2_S2A": "Zhenzhen GPT Image 2 S2A",
    ...
    "Comfly_Flux_2_Pro": "Zhenzhen_Flux_2_Pro",
    ...
    "Comfly_Doubao_Seedream_4_5": "Zhenzhen Doubao Seedream4.5",
    ...
    "Comfly_vidu_img2video": "Zhenzhen Vidu Image2Video",
    "Comfly_vidu_text2video": "Zhenzhen_vidu_text2video",
    "Comfly_vidu_ref2video": "Zhenzhen_vidu_ref2video",
    "Comfly_vidu_start-end2video": "Zhenzhen_vidu_start-end2video",
    ...
    "Comfly_Doubao_Seedance2_0_Asset": "Zhenzhen Doubao Seedance 2.0 Asset Upload",
